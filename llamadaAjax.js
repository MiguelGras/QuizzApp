    ...
    for (let i = 0; i < preguntas.length; i++) {
        if (numQuestion == i) {
            if (preguntas[i].type == "boolean") {
                ...
                <button class="btn btn-primary btn-lg btn-block" id="true" onclick="checkquestion(true); return false;" value="` + preguntas[i].correct_answer + `">` + preguntas[i].correct_answer + `</button>
                <button class="btn btn-primary btn-lg btn-block" id="false" onclick="checkquestion(false); return false;" value="` + preguntas[i].incorrect_answers[0] + `">` + preguntas[i].incorrect_answers[0] + `</button>
                <button class="btn btn-dark btn-lg btn-block" id='nextQuestion'>Next question</button>`
            } else {
                divPreguntas.innerHTML = `<h3>` + preguntas[i].question + `</h3>
                <button class="btn btn-primary btn-lg btn-block" id="true" onclick="checkquestion(true)"; return false;value="` + preguntas[i].correct_answer + `">` + preguntas[i].correct_answer + `</button>
                <button class="btn btn-primary btn-lg btn-block" id="false1" onclick="checkquestion(false)"; return false;" value="` + preguntas[i].incorrect_answers[0] + `">` + preguntas[i].incorrect_answers[0] + `</button>
                <button class="btn btn-primary btn-lg btn-block" id="false2" onclick="checkquestion(false); return false;" value="` + preguntas[i].incorrect_answers[1] + `">` + preguntas[i].incorrect_answers[1] + `</button>
                <button class="btn btn-primary btn-lg btn-block" id="false3" onclick="checkquestion(false); return false;" value="` + preguntas[i].incorrect_answers[2] + `">` + preguntas[i].incorrect_answers[2] + `</button>
                <button class="btn btn-dark btn-lg btn-block" id='nextQuestion'>Next question</button>`
            }
        }
